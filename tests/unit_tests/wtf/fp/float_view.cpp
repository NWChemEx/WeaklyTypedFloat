...
TEMPLATE_LIST_TEST_CASE("FloatView", "[wtf]", test_wtf::default_fp_types) {
...
TEST_CASE("FloatView (custom type)", "[wtf][custom_type]") {
    ...
    SECTION("Ctors and assignment") {
        SECTION("make_float_view") {
            REQUIRE(float_cast<float_t>(f) == val);
            REQUIRE(float_cast<float_t>(cf) == val);
        }
        ...
        SECTION("copy assignment") {
            float_t val2 = 0;
            view_type f2 = make_float_view(val2);
            auto pf2     = &(f2 = f);
            REQUIRE(f2 == f);
            REQUIRE(&float_cast<float_t&>(f2) == &val);
            REQUIRE(pf2 == &f2);

            const_view_type cf2 = make_float_view(std::as_const(val2));
            auto pcf2           = &(cf2 = cf);
            REQUIRE(cf2 == cf);
            REQUIRE(&float_cast<const float_t&>(cf2) == &std::as_const(val));
            REQUIRE(pcf2 == &cf2);
        }
        ...
        SECTION("move assignment") {
            ...
            const_view_type cf2 = make_float_view(std::as_const(val2));
            const_view_type cf3(cf);
            auto pcf2 = &(cf2 = std::move(cf3));
            REQUIRE(cf2 == cf);
            REQUIRE(pcf2 == &cf2);
        }
    ...
    SECTION("value()") {
        ...
        REQUIRE(cf.template value<float_t>() == val);
        REQUIRE(cf.template value<const float_t>() == val);
        REQUIRE(&cf.template value<const float_t&>() == &std::as_const(val));
